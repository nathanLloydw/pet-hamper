...
class WooSquareCashApp_Gateway extends WC_Payment_Gateway { 
	...
	public function __construct() {
		$this->id		            = 'square_cash_app_pay';
		$this->method_title 	    = __( 'Square Cash App Pay', 'wpexpert-square' );
		$this->method_description   = __( 'Square Cash App Pay works by adding payments button in an woocommerce checkout and then sending the details to Square for verification and processing.', 'wpexpert-square' );
	...
	public function init_form_fields() {
		$this->form_fields = apply_filters( 'woocommerce_square_cash_app_settings', array(
			'enabled' => array(
			...
			'description' => array(
				...
				'default'     => __( 'Pay with your Cash App via Square.', 'wpexpert-square')
			),
			'logging' => array(
	...
	public function payment_fields() { ?>
		<fieldset class="wooSquare-cash_app-payment">
		    <div id="payment-form">
	...
	public function payment_scripts_cash_app() {
		...
		if(!empty(get_transient('is_sandbox'))){
			wp_enqueue_script('cashapp_squareSDK', 'https://sandbox.web.squarecdn.com/v1/square.js', array(), WooSquare_VERSION);
			$environment =  'development'; 
		} else {
			wp_enqueue_script('cashapp_squareSDK', 'https://web.squarecdn.com/v1/square.js', array(), WooSquare_VERSION);
		...
		wp_enqueue_script( 'woosquare-cash-app-pay', WooSquare_PLUGIN_URL_PAYMENT . '/js/SquarePaymentsCashApp.js?rand='.rand(), array(), WooSquare_VERSION);
		wp_localize_script( 'woosquare-cash-app-pay', 'square_cashapp_params', array(
		...
		wp_enqueue_style( 'woocommerce-square-cashapp-styles', WooSquare_PLUGIN_URL_PAYMENT . '/css/SquareFrontendStyles_cash_app_pay.css' );
	...
	public function process_payment( $order_id, $retry = true ) {
		...
		try {
			...
			update_post_meta( $order->id, 'woosquare_request_results_cashapp_'.rand(1,1000), $result);
			...
			// }
			if (isset($result->payment->id) and $result->payment->source_type == 'WALLET' and $result->payment->status == 'COMPLETED' ) {
				...
				$complete_message = sprintf( __( 'Square CashApp Transaction complete '.$msg.' (Transaction ID: %s)', 'wpexpert-square' ), $result->payment->id );
				$order->add_order_note( $complete_message );
				$this->log( "Success: $complete_message" );				
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( 'square_cash_app_pay' === ( version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->payment_method : $order->get_payment_method() ) ) {
			try {
