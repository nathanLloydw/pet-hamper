...
class WooSquareGooglePay_Gateway extends WC_Payment_Gateway {
	...
	public function payment_scripts_googlepay() {
		...
		wp_enqueue_script('square-google-pay', 'https://'.$endpoint.'.squarecdn.com/v1/square.js', array(), '');
	...
	public function process_payment( $order_id, $retry = true ) {
		...
		update_post_meta( $order->id, 'errors_gpay', $errors ); 
		...
		if($woocommerce_square_google_pay_settings['capture'] === 'yes'){
			$capture = false;
		}else{
			$capture = true;
		}
