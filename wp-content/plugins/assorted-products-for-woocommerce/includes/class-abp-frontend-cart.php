			...
			if ( 'yes' == get_post_meta( $product_id, 'abp_enable_quantities_discounts', true ) ) {
				...
				if ( !empty($discounts['items']) && !empty($discounts['amount']) && $qties >= $discounts['items'] ) {
					if ( 'fixed' == $discounts['type'] ) {
						...
						$fee += $price / 100 * $discounts['amount'];
					}
				}
			}
			return $fee;
		}
		public function abp_get_extra_fee_price( $price, $product_id, $cart, $parent_key ) {
			if ( 'yes' == get_post_meta( $product_id, 'abp_per_item_extra_fee_enable', true ) ) {
				...
				if ( !empty($extra_fees) ) {
					foreach ( $cart as $cart_item_key => $cart_item ) {
						...
						if ( isset( $cart_item['abp_assorted_product_parent_key'] ) && $parent_key == $cart_item['abp_assorted_product_parent_key'] && isset($extra_fees[$item_id]) ) {
							$qty = $cart_item['quantity'];
							$extras += $qty * $extra_fees[$item_id]['fee'];
						}
					}
					$price += $extras;
				}
					...
					foreach ( $founds as $found ) {
						if ( $found['qty'] >= $found['items'] ) {
							if ( 'fixed' == $found['type'] ) {
								...
								$discount_amount = $price / 100 * $found['amount'];
							}
						}
					}
				}
			...
			if ( 'yes' == get_post_meta( $product_id, 'abp_enable_quantities_discounts', true ) ) {
				...
				if ( !empty($discounts['items']) && !empty($discounts['amount']) && $qties >= $discounts['items'] ) {
					if ( 'fixed' == $discounts['type'] ) {
						...
						$discount_amount += $price / 100 * $discounts['amount'];
					}
				}
			}
		...
		public function abp_assorted_product_before_calculate_totals( $cart_object ) {
			...
			foreach ( $cart_object->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( isset( $cart_item['abp-assorted-add-to-cart'] ) && ( '' != $cart_item['abp-assorted-add-to-cart'] ) ) {
					...
					$abp_assorted_product_price = apply_filters('abp_assorted_product_price_before_discounts', $abp_assorted_product_price, $product_id, $cart_item );
					...
					if ( 'yes' != get_option('abp_assorted_extra_fee_cart') ) {
						$abp_assorted_product_price = $this->abp_get_extra_fee_price( $abp_assorted_product_price, $product_id, $cart_object->get_cart(), $cart_item['key'] );
					}
					$abp_assorted_product_price = apply_filters('abp_assorted_product_final_price', $abp_assorted_product_price, $product_id, $cart_item );
			...
			if ( 'yes' == get_option('abp_assorted_extra_fee_cart') ) {
				...
				foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
					if ( isset( $cart_item['abp-assorted-add-to-cart'] ) && ( '' != $cart_item['abp-assorted-add-to-cart'] ) ) {
						$abp_assorted_product_items = explode( ',', $cart_item['abp-assorted-add-to-cart'] );
						$product_id  = $cart_item['product_id'];
						if ( is_array( $abp_assorted_product_items ) && count( $abp_assorted_product_items ) > 0 && 'yes' == get_post_meta( $product_id, 'abp_per_item_extra_fee_enable', true ) ) {
							$extra_fees = get_post_meta( $product_id, 'abp_per_item_extra_fees', true );
							foreach ( $abp_assorted_product_items as $item) {
								$item      = absint( $item );
								$qty = isset($cart_item['abp-assorted-item-quantity'][$item]) ? $cart_item['abp-assorted-item-quantity'][$item] : 1 ;
								...
								if ( isset($extra_fees[$item]) ) {
									$extras += $qty * $extra_fees[$item]['fee'];
								}
							}
						}
					}
				...
				if ( $extras > 0 ) {
					$label = get_option('abp_assorted_extra_fee_text', esc_html__('Extra Fee', 'wc-abp'));
					$cart->add_fee( esc_html( $label ), $extras, false );
				}
			}
		}
