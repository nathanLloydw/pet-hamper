	...
	private static function update_payment_limits() {
		...
		if (is_object($configuration)) {
			if (property_exists($configuration, 'errorCode')) {
				...
				if (isset($configuration->activeCountries) && is_array($configuration->activeCountries)) {
					$settings['active-countries'] = implode('|', $configuration->activeCountries);
					$settings_changed = true;
				} else {
					$settings['active-countries'] = 'N/A';
					$settings_changed = true;
